  ...
  return (
    <div className="flex-1 p-6 space-y-6 overflow-y-auto bg-slate-50/50 dark:bg-slate-950/50 transition-colors duration-300">
      <header className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight transition-colors">Catálogo Mestre</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm font-semibold uppercase tracking-widest text-[10px]">Gestão técnica de componentes, câmbios e marcas.</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-[11px] w-64 focus:ring-2 focus:ring-indigo-500 outline-none transition-all shadow-sm font-bold text-slate-900 dark:text-slate-100 placeholder-slate-400"
              value={searchTerm}
          ...
          {activeTab === 'gearboxes' && (
            <>
              <select
                ...
                className="px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-[11px] focus:ring-2 focus:ring-indigo-500 outline-none transition-all shadow-sm font-bold text-slate-700 dark:text-slate-200 uppercase"
              >
              ...
              <button
                ...
                className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all"
              >
          ...
          <button
            ...
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all shadow-lg shadow-indigo-100 dark:shadow-indigo-900/20 active:scale-95"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        {stats.map((s, i) => (
          <div key={i} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-3 transition-all hover:scale-[1.02]">
            <div className={`p-3 rounded-xl ${s.bg} dark:bg-opacity-10`}>
              <s.icon className={`w-5 h-5 ${s.color}`} />
            </div>
            <div>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{s.label}</p>
              <p className="text-lg font-black text-slate-900 dark:text-slate-100 leading-none mt-1 uppercase tracking-tight">{s.value}</p>
            </div>
          </div>
        ))}
      </div>

      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm flex flex-col transition-all">
        <div className="flex border-b border-slate-100 dark:border-slate-700/50 bg-slate-50/30 dark:bg-slate-800/50 overflow-x-auto">
          <button
            onClick={() => { setActiveTab('parts'); setSearchTerm(''); }}
            className={`px-8 py-4 text-[10px] font-black transition-all flex items-center gap-2 uppercase tracking-widest border-b-2 shrink-0 ${activeTab === 'parts' ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 border-indigo-600 dark:border-indigo-500' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 border-transparent'
              }`}
          >
          ...
          <button
            ...
            className={`px-8 py-4 text-[10px] font-black transition-all flex items-center gap-2 uppercase tracking-widest border-b-2 shrink-0 ${activeTab === 'gearboxes' ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 border-indigo-600 dark:border-indigo-500' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 border-transparent'
              }`}
          >
          ...
          <button
            ...
            className={`px-8 py-4 text-[10px] font-black transition-all flex items-center gap-2 uppercase tracking-widest border-b-2 shrink-0 ${activeTab === 'brands' ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 border-indigo-600 dark:border-indigo-500' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 border-transparent'
              }`}
          >
          ...
          <button
            ...
            className={`px-8 py-4 text-[10px] font-black transition-all flex items-center gap-2 uppercase tracking-widest border-b-2 shrink-0 ${activeTab === 'services' ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 border-indigo-600 dark:border-indigo-500' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 border-transparent'
              }`}
          >
          ...
          <button
            ...
            className={`px-8 py-4 text-[10px] font-black transition-all flex items-center gap-2 uppercase tracking-widest border-b-2 shrink-0 ${activeTab === 'payment-methods' ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 border-indigo-600 dark:border-indigo-500' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 border-transparent'
              }`}
          >
                ...
                {filteredInventory.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{item.name}</div>
                      <div className="flex gap-2 mt-0.5">
                        <span className="text-[9px] font-black text-slate-300 dark:text-slate-600 bg-slate-100 dark:bg-slate-800 px-1 rounded">#{item.id}</span>
                        <span className="text-[9px] font-mono text-slate-400 uppercase">{item.code}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{item.supplier}</td>
                    <td className="px-6 py-4 text-center">
                      <span className={`text-xs font-black ${item.stock < item.minStock ? 'text-red-600 dark:text-red-400' : 'text-slate-700 dark:text-slate-300'}`}>{item.stock}</span>
                      <span className="text-[9px] text-slate-400 ml-1 font-bold">/ {item.minStock}</span>
                    </td>
                    <td className="px-6 py-4 text-xs text-indigo-600 dark:text-indigo-400 font-black text-right font-mono tracking-tighter">R$ {item.salePrice.toLocaleString()}</td>
                    <td className="px-6 py-4">
                      <div className="flex justify-center gap-2">
                        <button onClick={() => handleOpenModal(item)} className="p-1.5 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"><Edit3 className="w-4 h-4" /></button>
                        <button onClick={() => handleDelete(item.id)} className="p-1.5 text-slate-400 hover:text-red-500 dark:hover:text-red-400 transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            <table className="w-full text-left min-w-[800px]">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                {filteredBrands.map(brand => (
                  <tr key={brand.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors group">
                    <td className="px-6 py-4">
                      <span className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase">{brand.name}</span>
                      <span className="ml-2 text-[9px] font-black text-slate-300 dark:text-slate-600 bg-slate-100 dark:bg-slate-800 px-1 rounded">#{brand.id}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex justify-center gap-2">
                        <button onClick={() => handleDelete(brand.id)} className="p-1.5 text-slate-400 hover:text-red-500 dark:hover:text-red-400 transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            <table className="w-full text-left min-w-[800px]">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                {filteredGearboxes.map(gearbox => (
                  <tr key={gearbox.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="text-xs font-black text-indigo-600 dark:text-indigo-400 uppercase font-mono tracking-widest">{gearbox.code}</div>
                      <span className="text-[9px] font-black text-slate-300 dark:text-slate-600 bg-slate-100 dark:bg-slate-800 px-1 rounded block w-fit mt-1">#{gearbox.id}</span>
                    </td>
                    <td className="px-6 py-4 text-xs font-bold text-slate-700 dark:text-slate-200 uppercase">{gearbox.brand}</td>
                    <td className="px-6 py-4 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{gearbox.model}</td>
                    <td className="px-6 py-4"><span className="bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 text-[9px] font-black px-2 py-0.5 rounded border border-amber-200 dark:border-amber-700/50 uppercase tracking-widest">{gearbox.assemblyTime || '-'}</span></td>
                    <td className="px-6 py-4">
                      <div className="flex justify-center gap-2">
                        <button onClick={() => handleOpenModal(gearbox)} className="p-1.5 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"><Edit3 className="w-4 h-4" /></button>
                        <button onClick={() => handleDelete(gearbox.id)} className="p-1.5 text-slate-400 hover:text-red-500 dark:hover:text-red-400 transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            <table className="w-full text-left min-w-[800px]">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                {filteredServices.map(service => (
                  <tr key={service.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{service.name}</div>
                      <div className="flex gap-2 mt-0.5">
                        <span className="text-[9px] font-black text-slate-300 dark:text-slate-600 bg-slate-100 dark:bg-slate-800 px-1 rounded">#{service.id}</span>
                        <span className="text-[9px] font-mono text-slate-400 uppercase">{service.code}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-xs text-emerald-600 dark:text-emerald-400 font-black text-right font-mono tracking-tighter">R$ {service.price.toLocaleString()}</td>
                    <td className="px-6 py-4">
                      <div className="flex justify-center gap-2">
                        <button onClick={() => handleOpenModal(service)} className="p-1.5 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"><Edit3 className="w-4 h-4" /></button>
                        <button onClick={() => handleDelete(service.id)} className="p-1.5 text-slate-400 hover:text-red-500 dark:hover:text-red-400 transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            <table className="w-full text-left min-w-[800px]">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                {paymentMethods?.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase())).map(method => (
                  <tr key={method.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors group">
                    <td className="px-6 py-4">
                      <span className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase">{method.name}</span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <button
                        onClick={() => togglePaymentMethod(method.id, !method.active)}
                        className={`px-2 py-1 rounded text-[9px] font-black uppercase tracking-wider ${method.active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400'}`}
                      >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-center gap-2">
                        <button onClick={() => handleDelete(method.id)} className="p-1.5 text-slate-400 hover:text-red-500 dark:hover:text-red-400 transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setItemToDelete(null)}>
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl overflow-hidden max-w-sm w-full p-6 animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <div>
                <h3 className="text-lg font-black text-slate-900 dark:text-slate-100 leading-tight mb-2">Excluir Registro?</h3>
              ...
              <div className="grid grid-cols-2 gap-3 w-full pt-2">
                ...
                <button
                  className="w-full py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold text-xs uppercase tracking-widest rounded-xl transition-colors shadow-lg shadow-red-100 dark:shadow-red-900/20"
...
const InventoryModal: React.FC<InventoryModalProps> = ({ type, item, onClose, context }) => {
  ...
  const inputClasses = "w-full p-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 outline-none transition-colors";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700 transition-colors" onClick={e => e.stopPropagation()}>
        <header className="px-6 py-4 border-b border-slate-100 dark:border-slate-700/50 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
          <div>
            <h3 className="text-sm font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight">{item ? 'Editar' : 'Novo'} {type === 'parts' ? 'Peça' : type === 'services' ? 'Serviço' : type === 'gearboxes' ? 'Câmbio' : type === 'payment-methods' ? 'Pagamento' : 'Marca'}</h3>
            <p className="text-[9px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest mt-0.5">Gestão de Catálogo</p>
          </div>
          <button onClick={onClose} className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg transition-colors"><X className="w-4 h-4 text-slate-400 dark:text-slate-500" /></button>
        </header>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className={type === 'services' ? "col-span-2" : "col-span-1"}>
                ...
                <input
                  ...
                  className={`${inputClasses} ${type !== 'brands' && type !== 'payment-methods' ? 'bg-slate-100 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 cursor-not-allowed' : ''}`}
          ...
          <button type="submit" className="w-full py-3 bg-indigo-600 text-white font-black text-[10px] uppercase tracking-widest rounded-xl shadow-lg shadow-indigo-100 dark:shadow-indigo-900/20 hover:bg-indigo-700 active:scale-[0.98] transition-all flex items-center justify-center gap-2 mt-4">
